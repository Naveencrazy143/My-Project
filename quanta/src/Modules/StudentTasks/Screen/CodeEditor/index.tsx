...
import { languageOptions, showToast } from "@Utils";
...
const CodeEditor = React.forwardRef(({ ref, onSubmit, value, showAddSubmit = false, isLoading, showExpand = false, codeOutput, codeSubmit, courseIde, isFromStudentTask = false, isMyTask = false, ...props }: Props) => {
    ...
    const [outputDetails, setOutputDetails] = useState<any>(codeOutput ? codeOutput : null);
    ...
    const [statusButton, setStatusButton] = useState(icons.playButton)
    ...
    console.log("codeOutput========", javaScript)
    ...
    const handleCompile = () => {
        ...
        if (courseIde === "HTML") {
            console.log("courseide", courseIde)
            setAllProgramData(JSON.stringify({
                js: javaScript,
                css: styleCSS,
                html: htmlValue
            }))
            const iframe: any = document.getElementById('output') as HTMLIFrameElement;
    ...
    `;
        ...
        `);
            ...
            setPreviousCode(isBase64(javaScript) ? convertToBase64(javaScript) : window.btoa(javaScript))

            const params = {
                source_code: isBase64(javaScript) ? convertToBase64(javaScript) : window.btoa(javaScript),
            ...
            dispatch(postStudentCodeSubmission({
                params,
                onSuccess: (success: any) => () => {
                    ...
                    if (success?.status?.id === 11) {
                        ...
                        setTimeout(() => {
                            setStatusButton(icons.playButton)
                        }, 1500);
                    }
                    else {
                    ...
                    dispatch(setCodeOutputData(success))
        ...
        <div>
            <div className={`row  mx-1 `}>
                <div className={`col-sm-6`} style={{
                    paddingRight: '0px',
                    paddingLeft: "0px",
                    borderRight: themeMode ? "1px solid #808080" : " 2px solid black",
                    borderBottom: "",
                    borderLeft: ''
                }}>
                    {
                        <div className={`py-1 ${isExpandCodeEditor ? "bg-white" : 'bg-white'} ${themeMode ? "" : "border-black"}`} style={{ border: " 2px solid #808080", padding: '-5px 0px -5px 0px' }}>
                            <div className=''>

                                <div className='row'>
                                    {courseIde !== "HTML" &&
                                        <div className=' row ml-4 '>
                                            <img src={icons.defaultImage} height={23} className="" alt='...' />
                                            <div className={'font-weight-bold ml-1 mt-1'} style={{ fontSize: '12px', }}>{'Editor'}</div>
                                        </div>}
                                    {courseIde === "HTML" &&
                                        <div className='row ml-4'>
                                            <Button
                                    ...
                                    <div className=" col  text-right  mr-sm-2" >
                                        ...
                                        {/* theme image */}
                                        <Image src={themeMode ? icons.dark : icons.moon} height={25}
                                            className="px-2  mr-2"
                                            style={{ border: '2px solid rgba(0, 0, 0, 0.05)' }}
                                            onClick={() => {
                                                setThemeMode(!themeMode)
                                                if (!themeMode) {
                                                    handleThemeChange({ label: 'krTheme', value: 'vs-dark', key: 'krtheme' })
                                                }
                                                else {
                                                    handleThemeChange({ label: 'Active4D', value: 'active4d', key: 'active4d' })
                                        ...
                                        {processing ?
                                            <RsSpinner
                                                ...
                                                aria-hidden="true"
                                                animation="grow" />
                                            : <Image src={statusButton} height={23} className="mr-3 "
                                                onClick={() => {
                                                    if (setFocus) {
                                                        setFocus('')
                                                    }
                                                    ...
                                                    if (javaScript || styleCSS || htmlValue) {
                                                        ...
                                                        showToast("error", 'Please type some code to run')
                                                    }
                                                }
                                                }
                                            />
                ...
                <div className={`col-sm-6 ${isExpandCodeEditor ? '' : "pt-sm-0 pt-xl-0 pt-3"}`} style={{ paddingLeft: '0px', paddingRight: '0px', }}>
                    {courseIde === "HTML" && <div className={` ${isExpandCodeEditor ? "bg-white" : 'bg-white'}`} style={{ backgroundColor: "#1e1e1e", border: " 2px solid #808080", padding: '18px 0px 18px 0px' }}>
                        {/* <Button
                            text={"run"}
            ...
            </div>
            {
                ((isMyTask || !showAddSubmit && !isExpandCodeEditor && outputDetails?.status?.id !== 11 && outputDetails !== null) || (courseIde === "HTML" && !isExpandCodeEditor && !showAddSubmit)) && <div className="text-right mt-2 pb-3">
                    <Button
                        text={isMyTask ? 'Save' : translate('course.submitForApproval')}
                        size={'md'}
                        isLoading={isLoading}
                        onClick={() => {
                            if (setFocus)
                            ...
                            if (onSubmit) {
                                ...
                                if (courseIde === "HTML") {
                                    ...
                                    console.log("programdatata==>", programData)
