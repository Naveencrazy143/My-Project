...
const LaunchScreenSecondary = () => {
  ...
  const [displaySeconds, setDisplaySeconds] = useState(Math.floor(timestamp) > timer ? Math.floor(timestamp) : timer);
    ...
    >
      ...
      </div>
      {
        <div className="container-fluid ml-xl-5 ml-sm-0 ml-3">
          <div className="text-white h1 ml-1 fs-3 mt-sm-0 mt-9" >{activeUser ? `Enter Launch Code by ${activeUser.name}` : <><b>{"ZenyQ"}</b> {"launching in"}</>}</div>
          ...
          {activeUser && <> <div className="text-white h3 mt-5 fw-normal" >{"Enter your code here"}</div>
            <Container
              flexDirection={'row'}
              margin={'mt-2'}
              additionClass={''}
